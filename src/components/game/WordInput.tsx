...
const WordInput = ({ 
  ...
  const handleWordChange = (value: string) => {
    // Clean the input by removing whitespace, special characters, and converting to lowercase
    const cleanedWord = value.trim().toLowerCase().replace(/[^a-z]/g, '');
    onWordChange(cleanedWord);
  };

  // Memoize the placeholder text to prevent unnecessary re-renders
  const placeholderText = useMemo(() => 
    editingIndex !== null ? `Change word #${editingIndex + 1}` : "Enter a word...",
    [editingIndex]
  );
